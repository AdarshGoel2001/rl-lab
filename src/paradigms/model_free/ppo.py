...
@register_algorithm("paradigm_ppo")
class PPOAlgorithm:
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize storage for components and networks
        self.networks = {}
        self.optimizers = {}
        self.step = 0
    ...
    def set_components(self, encoder, policy_head, value_function):
        """
        Set modular components and create composite networks.
        Called by trainer after creating components.
        """
        # Store components
        ...
        # Create composite networks (combines encoder + head)
        self.networks['actor'] = CompositeActor(self.encoder, self.policy_head)
        self.networks['critic'] = CompositeCritic(self.encoder, self.value_function)
    ...
    def get_metrics(self) -> Dict[str, float]:
        """Get PPO-specific metrics for logging"""
        return {
    ...
    def get_state(self) -> Dict[str, Any]:
        """
        Get complete algorithm state for checkpointing.

        Returns:
            Dictionary containing all algorithm state including networks,
            optimizers, hyperparameters, and training progress.
        """
        return {
    ...
    def set_state(self, state: Dict[str, Any]):
        ...
        Args:
            state: Dictionary containing algorithm state from get_state()
        """
        # Restore network weights
        if 'networks' in state:
            for name, state_dict in state['networks'].items():
        ...
        if 'hyperparameters' in state:
            hparams = state['hyperparameters']
            self.clip_ratio = hparams.get('clip_ratio', self.clip_ratio)
            self.value_coef = hparams.get('value_coef', self.value_coef)
            self.entropy_coef = hparams.get('entropy_coef', self.entropy_coef)
    ...
    # Methods for trainer compatibility
    def train(self):
...
class CompositeActor(nn.Module):
    """Combines encoder + policy_head to create a complete actor network."""
...
class CompositeCritic(nn.Module):
    """Combines encoder + value_function to create a complete critic network."""

    def __init__(self, encoder, value_function):
        super().__init__()
        self.encoder = encoder
        self.value_function = value_function
