...
Provides the shared training orchestration logic used by all paradigms.
Paradoxically sits outside any specific paradigm so model-free, world-model,
and future agents can inherit the same experiment plumbing while customizing
their component creation.
"""
...
class BaseParadigmTrainer(RegistryMixin, ABC):
    """
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            self.is_vectorized_env = is_vectorized_env

            logger.info("Initializing paradigm/agent via subclass hook...")
            self.algorithm = self._create_paradigm(obs_space, action_space, device)
            if self.algorithm is None:
                raise ValueError("_create_paradigm must return a configured paradigm/agent instance")

            if hasattr(self.algorithm, 'to'):
                self.algorithm.to(device)
            logger.info(f"Created paradigm/agent: {self.algorithm.__class__.__name__}")
            
            # Initialize buffer
            logger.info("Initializing buffer...")
            buffer_config = self.config.buffer.__dict__.copy()
            buffer_config['device'] = device
    ...
    @abstractmethod
    def _create_paradigm(self,
                          obs_space: Any,
                          action_space: Any,
                          device: torch.device):
        """Create and return the paradigm/agent instance for this trainer."""
    ...
    def _collect_vectorized_experience(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        while steps_collected < buffer_capacity and self.step < self.config.training.total_timesteps:
            ...
            if isinstance(next_obs, torch.Tensor):
                next_observations.append(next_obs.cpu().numpy())
            else:
